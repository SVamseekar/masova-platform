...
public class DriverAcceptanceService {
    ...
    /**
     * Mark delivery as delivered (simple flow without OTP/photo verification)
     * This is used by the driver app for quick delivery completion.
     * For more secure verification, use the ProofOfDeliveryService endpoints.
     */
    public DeliveryTracking markAsDelivered(String trackingId, String driverId, String notes) {
        DeliveryTracking tracking = deliveryTrackingRepository.findById(trackingId)
                .orElseThrow(() -> new RuntimeException("Delivery tracking not found: " + trackingId));
        ...
        tracking.setUpdatedAt(deliveredAt);

        // Calculate actual delivery time if picked up time is available
        if (tracking.getPickedUpAt() != null) {
            ...
            // Check if on time based on estimated delivery minutes
        ...
        // Also update the order status to DELIVERED via Order Service
        try {
            orderServiceClient.markOrderDelivered(tracking.getOrderId(), deliveredAt, "QUICK_DELIVERY");
            log.info("Order {} status updated to DELIVERED", tracking.getOrderId());
        } catch (Exception e) {
            log.error("Failed to update order status to DELIVERED: {}", e.getMessage());
            // Don't throw - the delivery tracking is already updated
        }
        ...
        Map<String, Object> notification = Map.of(
                ...
                "orderId", tracking.getOrderId(),
                "driverName", tracking.getDriverName() != null ? tracking.getDriverName() : "Driver",
                "message", "Your order has been delivered! Thank you for ordering with us.",
        ...
        log.info("Delivery {} marked as delivered by driver {} at {}", trackingId, driverId, deliveredAt);
