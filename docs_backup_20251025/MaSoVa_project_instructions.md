...
# 2-6. Move through all stages
curl -X PATCH http://localhost:8083/api/orders/$ORDER_ID/next-stage  # PREPARING
curl -X PATCH http://localhost:8083/api/orders/$ORDER_ID/next-stage  # OVEN
curl -X PATCH http://localhost:8083/api/orders/$ORDER_ID/next-stage  # BAKED
curl -X PATCH http://localhost:8083/api/orders/$ORDER_ID/next-stage  # DISPATCHED
curl -X PATCH http://localhost:8083/api/orders/$ORDER_ID/next-stage  # DELIVERED
```

---

## UX Guidelines - OrderForm Component

### Quantity Controls UX Pattern

Items start at quantity = 0 and must be explicitly added to cart:

**Initial State (Not in Cart):**
```
┌─────────────────────┐
│ Margherita Pizza    │
│ ₹299.00             │
...
**Benefits:**
- ✅ Prevents Accidental Additions - Users must explicitly click "Add"
- ✅ Clear Intent - Obvious visual feedback when item is in cart
- ✅ Better UX - Follows e-commerce best practices
- ✅ Easy Removal - Clicking "−" when quantity is 1 removes the item
- ✅ Visual Clarity - Green "Add" button vs quantity controls

**Implementation Details:**
- File: `frontend/src/components/forms/OrderForm.tsx`
- Helper function: `getItemQuantity()` returns 0 if not in cart
- Decrement function: `decrementItem()` removes item when quantity reaches 0
- CSS: Neumorphic design with green "Add" button and red quantity controls
...
frontend/
├── src/
│   ├── apps/                  # Segregated applications
│   │   ├── POSSystem/        # POS application
│   │   ├── PublicWebsite/    # Public-facing site
...
VITE_MENU_SERVICE_URL=http://localhost:8082
...
**Document Updated:** October 25, 2025
**Status:** Consolidated with Quick Start, UX Guidelines, and Frontend Setup
