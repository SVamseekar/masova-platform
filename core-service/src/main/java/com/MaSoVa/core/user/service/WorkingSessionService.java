...
public class WorkingSessionService {
    ...
    public WorkingSession clockInWithPin(String employeeId, String pin, String storeId, String managerId) {
        ...
        // If employee is a DRIVER, automatically set their status to AVAILABLE
        if (employee.getType() == com.MaSoVa.shared.enums.UserType.DRIVER) {
            employee.getEmployeeDetails().setStatus("AVAILABLE");
            userRepository.save(employee);
            logger.info("Driver {} status set to AVAILABLE on clock-in", employeeId);
        }
    ...
    public WorkingSession endSessionWithLocation(String employeeId, Location clockOutLocation) {
        ...
        // If employee is a DRIVER, set their status to OFF_DUTY on clock-out
        User employee = userRepository.findById(employeeId).orElse(null);
        if (employee != null && employee.getType() == com.MaSoVa.shared.enums.UserType.DRIVER) {
            if (employee.getEmployeeDetails() != null) {
                employee.getEmployeeDetails().setStatus("OFF_DUTY");
                userRepository.save(employee);
                logger.info("Driver {} status set to OFF_DUTY on clock-out", employeeId);
            }
        }
