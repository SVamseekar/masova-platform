...
class UserServiceTest {
    ...
    class GetUserStatistics {
        ...
        @Test
        @DisplayName("returns stats map with totalUsers, activeUsers, usersByType")
        void returnsStatsMap() {
            when(userRepository.count()).thenReturn(50L);
            when(userRepository.countByIsActive(true)).thenReturn(40L);
            when(userRepository.countByLastLoginAfter(any())).thenReturn(10L);
            when(userRepository.countByType(any())).thenReturn(5L);

            Map<String, Object> stats = userService.getUserStatistics();

            assertThat(stats).containsKey("totalUsers");
            assertThat(stats.get("totalUsers")).isEqualTo(50L);
            assertThat(stats.get("activeUsers")).isEqualTo(40L);
            assertThat(stats.get("inactiveUsers")).isEqualTo(10L);
            assertThat(stats).containsKey("usersByType");
        }
    }
    ...
    class GetDriverStatsByStore {
        ...
        @Test
        @DisplayName("returns driver stats with counts for a store")
        void returnsDriverStats() {
            User driver = buildUser("d1", "driver@masova.com", UserType.DRIVER);
            User.EmployeeDetails details = new User.EmployeeDetails();
            details.setStoreId("store-1");
            ...
            assertThat(stats).containsKey("totalDrivers");
            assertThat(stats.get("totalDrivers")).isEqualTo(1L);
            assertThat(stats.get("onlineDrivers")).isEqualTo(1L);
        }

        @Test
        @DisplayName("returns zero counts when no drivers for store")
        void returnsZeroWhenEmpty() {
            when(userRepository.findByTypeAndEmployeeDetailsStoreId(UserType.DRIVER, "empty-store"))
                    .thenReturn(List.of());

            Map<String, Object> stats = userService.getDriverStatsByStore("empty-store");

            assertThat(stats.get("totalDrivers")).isEqualTo(0L);
            assertThat(stats.get("activeDrivers")).isEqualTo(0L);
        }
    }
    ...
    class VerifyEmployeePIN {
        ...
        @Test
        @DisplayName("returns false when employee has no PIN set")
        void returnsFalseWhenNoPinSet() {
            User emp = buildEmployee("emp-1", "emp@masova.com", "store-1");
            emp.getEmployeeDetails().setEmployeePINHash(null);
            when(userRepository.findById("emp-1")).thenReturn(Optional.of(emp));

            assertThat(userService.verifyEmployeePIN("emp-1", "12345")).isFalse();
        }

        @Test
        @DisplayName("returns true when PIN matches")
        void returnsTrueForCorrectPin() {
            User emp = buildEmployee("emp-1", "emp@masova.com", "store-1");
            emp.getEmployeeDetails().setEmployeePINHash("hashed");
            when(userRepository.findById("emp-1")).thenReturn(Optional.of(emp));
            when(passwordEncoder.matches("12345", "hashed")).thenReturn(true);

            assertThat(userService.verifyEmployeePIN("emp-1", "12345")).isTrue();
        }

        @Test
        @DisplayName("returns false when PIN does not match")
        void returnsFalseForWrongPin() {
            User emp = buildEmployee("emp-1", "emp@masova.com", "store-1");
            emp.getEmployeeDetails().setEmployeePINHash("hashed");
            when(userRepository.findById("emp-1")).thenReturn(Optional.of(emp));
            when(passwordEncoder.matches("12345", "hashed")).thenReturn(false);
    ...
    // ===========================
    // findUserByPIN — optimized suffix lookup
    // ===========================
    ...
    class FindUserByPIN {
        ...
        @Test
        @DisplayName("returns null when PIN is null")
        void returnsNullForNullPin() {
            assertThat(userService.findUserByPIN(null)).isNull();
        }

        @Test
        @DisplayName("returns null when PIN is wrong length")
        void returnsNullForWrongLength() {
            assertThat(userService.findUserByPIN("123")).isNull();
        }

        @Test
        @DisplayName("returns matching user when suffix lookup finds correct candidate")
        void returnsUserOnSuffixMatch() {
            User emp = buildEmployee("emp-1", "emp@masova.com", "store-1");
            emp.getEmployeeDetails().setEmployeePINHash("hashed");
            when(userRepository.findByEmployeeDetailsPinSuffix("45")).thenReturn(List.of(emp));
            when(passwordEncoder.matches("12345", "hashed")).thenReturn(true);

            User result = userService.findUserByPIN("12345");
        ...
        @Test
        @DisplayName("falls back to full scan when suffix returns no candidates")
        void fallsBackToFullScanWhenNoSuffixMatch() {
            when(userRepository.findByEmployeeDetailsPinSuffix("45")).thenReturn(List.of());
            when(userRepository.findByTypeIn(any())).thenReturn(List.of());

            User result = userService.findUserByPIN("12345");

            assertThat(result).isNull();
            verify(userRepository).findByTypeIn(any());
        }
    }
}
